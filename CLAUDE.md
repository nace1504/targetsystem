...
Đọc theo thứ tự khi cần context: `docs/00-tong-quan.md` → `docs/01-requirements.md` (yêu cầu) → `docs/02-design.md` (kiến trúc mục tiêu) → **`docs/04-detailed-design.md` + `docs/05-detailed-design-part2.md` + `docs/06-detailed-design-part3.md` (BẮT BUỘC đọc cả 3 trước khi code bất kỳ module nào — schema/API chính xác từng field, rule severity, data flow 7 kịch bản, network/deploy, test fixtures cố định, system prompt LLM literal, HMAC signing, tracing schema, lớp ingest. Không tự bịa field/rule/tên host/tên document/prompt nào khác với 2 file này)** → `docs/03-checklist.md` (trạng thái build thật) → `PLAN.md` (thứ tự làm + mốc thời gian). Tài liệu nguồn gốc của dự án tổng (đề bài, PRD, TRD, testbed spec, kế hoạch team) nằm ở `docs/source/` — chỉ đọc khi cần đối chiếu chi tiết, các file trong `docs/` ở trên đã là bản tóm tắt sạch.
